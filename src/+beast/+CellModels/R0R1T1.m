...
classdef R0R1T1 < CellModels.CellModel
    % deltat FIXED
...
methods
    ...
    % Initialization
    function obj = R0R1T1( CellModelData, COV, deltat )
...
methods(Static)    
    ...
    function CheckCellModelDim( AA )
        if CellModels.R0R1T1.Nx ~= length(AA.x0)
            disp 'ERROR in CellModel - 10 Nx'
            pause
        end
        if CellModels.R0R1T1.Np ~= length(AA.p0)
        	disp 'ERROR in CellModel - 20 Np'
            pause
        end
        if CellModels.R0R1T1.Nu ~= length(AA.u_all(:,1))
